...
class _AmortizationCalculatorState extends State<AmortizationCalculator> {
  ...
  void dispose() {
    ...
    try {
      ...
      // No dividimos la tasa entre 100 aquí ya que lo haremos en los métodos específicos
      final rate = double.parse(_rateController.text.replaceAll(',', '.'));
      final periods = int.parse(_periodsController.text);      switch (_amortizationMethod) {
        case 'french':
  ...
  void _calculateAmericanAmortization(
    ...
    // Calculamos el pago mensual de intereses (tasa mensual directa)
    double monthlyRate = rate / 100; // Convertir porcentaje a decimal
